...
describe("detectServerProtocol", () => {
  test("detects V2 by its process identifier even when both API generations exist", async () => {
  ...
  test("prefers the legacy health endpoint when the current API lacks a process identifier", async () => {
    const fetcher = mockFetch((input) => {
      const path = new URL(input instanceof Request ? input.url : input).pathname
      if (path === "/global/health") return Promise.resolve(json({ healthy: true, version: "1.18.4" }))
      return Promise.resolve(json({ healthy: true }))
